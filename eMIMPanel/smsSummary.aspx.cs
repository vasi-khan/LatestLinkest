...
namespace eMIMPanel
{
    public partial class smsSummary : System.Web.UI.Page
    {
        ...
        public void GetData()
        {
            ...
            string username = txtProfile.Text.Trim();
            ...
            if (username != "")
            {
                ...
                else if ((usertype.ToUpper() == "BD" && Convert.ToString(Session["EMPCODE"]) != ob.ValidateEmpCode(username)))
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('User does not belong to this admin. !!')", true);
                    return;
                }
            ...
            {
                dt = ob.GetDayWiseSMSSummary(username, s1, s2, txtSenderId.Text, "","","");
            }
            else if(usertype == "BD")
            {
                dt = ob.GetDayWiseSMSSummary(username, s1, s2, txtSenderId.Text, "", usertype, Convert.ToString(Session["EMPCODE"]));
            }
            else
            {
                dt = ob.GetDayWiseSMSSummary(username, s1, s2, txtSenderId.Text, Convert.ToString(Session["DLT"]), usertype," ");
            }
            ViewState["dt"] = dt;
            grv.DataSource = null;
            grv.DataSource = dt;
