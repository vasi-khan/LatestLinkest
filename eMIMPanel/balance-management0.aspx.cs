...
namespace eMIMPanel
{
    public partial class balance_management0 : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (rbWABA.Checked)
            {
                DataTable dtW = ob.GetUserParameter(un);
                if (dtW.Rows.Count > 0)
                {
                    if (dtW.Rows[0]["WABARCS"] == DBNull.Value || Convert.ToBoolean(dtW.Rows[0]["WABARCS"]) == false)
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('This User (" + un + ") does not have authorized for this activity (WABA RCS)');", true);
                        return;
                    }
                }
            ...
            string r = "";
            if (rbClick.Checked)
                r = ob.SaveClickCrDrBalance(un, bal, cd, _user, usertype, remarks);
            else if (rbWABA.Checked)
                r = ob.SaveWABARCSBalance(un, bal, cd, _user, usertype, remarks);
            else
                r = ob.SaveCrDrBalance(un, bal, cd, _user, usertype, remarks);

            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + r + "');window.location ='balance-management0.aspx';", true);
        }
