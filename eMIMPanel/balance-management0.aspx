<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="balance-management0.aspx.cs" Inherits="eMIMPanel.balance_management0" MaintainScrollPositionOnPostback="true" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="modal fade" id="modal-balance" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="h6 modal-title mb-0" id="modal-title-default" style="margin-right: 55px">Balance Management</h2>
                    <%--  <asp:RadioButtonList ID="rblFilter" runat="server" RepeatDirection="Horizontal">
                        <asp:ListItem style="margin-right: 25px" Text="Balance" Value="1" Selected="True"></asp:ListItem>
                        <asp:ListItem Text="Click Balance" Value="2"></asp:ListItem>
                        <asp:ListItem Text="WABA RCS" Value="3"></asp:ListItem>
                    </asp:RadioButtonList>--%>
                    <asp:RadioButton ID="rbBalance" runat="server" Checked="true" Text="Balance" GroupName="rbl" Style="margin-right: 25px" />
                    <asp:RadioButton ID="rbClick" runat="server" Text="Click Balance" GroupName="rbl" Style="margin-right: 25px" />
                    <div id="divWABA">
                        <asp:RadioButton ID="rbWABA" runat="server" Text="WABA RCS" GroupName="rbl" />
                    </div>
                ...
                <div class="modal-body">
                    <div class="form-row">
                        ...
                        <div class="col-md-12">
                            <label for="validationCustom02" class="font-weight-bold">Remarks</label>
                            <%--<input type="number" class="form-control" id="validationCustom01" value="">--%>
                            <asp:TextBox ID="txtRemarks" runat="server" class="form-control" Text="" />
                            <div class="valid-feedback">Looks good! </div>
                        </div>
                    </div>
    ...
    <script type="text/javascript">  
        function UpdateRate() {
            ...
            if (s1 == "" && s2 == "" && s3 == "" && s4 == "" && s5 == "" && s6 == "") {
                ...
                var urlrate = '<% =Session["URLRATE"].ToString() %>';
                var dltch = '<% =Session["DLTCHARGE"].ToString() %>';
                var normalrate = '<% =Session["RATE_NORMALSMS"].ToString() %>';
                var linkrate ='<% =Session["RATE_SMARTSMS"].ToString() %>';
                var camprate ='<% =Session["RATE_CAMPAIGN"].ToString() %>';
                var otprate = '<% =Session["RATE_OTP"].ToString() %>';
                if (usertype == "ADMIN") {
                    if (parseFloat(s1) < parseFloat(normalrate) || parseFloat(s2) < parseFloat(linkrate) || parseFloat(s3) < parseFloat(camprate) || parseFloat(s4) < parseFloat(otprate) || parseFloat(s5) < parseFloat(urlrate) || parseFloat(s6) < parseFloat(dltch)) {
                        alert('New entered price cannot be less than price defined for Admin.');
                        return false;
                    }
                    else {
                        return true;
                        ...
                        columns: [
                            { 'data': 'Sln' },
                            {
                                'data': null, 'bSortable': false, 'mRender': function (data, type, full) {
                                    //return '<a href="#" onclick="CheckAccess(MIM2000003);" class="mx-1 btn btn-primary text-secondary" data-toggle="modal" data-id="' + full.UserName + '" data-target="#modal-balance"> <i class="far fa-credit-card"></i></a>';
                                    return '<a href="#" onclick="CheckAccess(\'' + full.UserName + '\');" class="mx-1 btn btn-primary text-secondary" data-toggle="modal" data-id="' + full.UserName + '" data-target="#modal-balance"> <i class="far fa-credit-card"></i></a>';
                                   
                                }
                            },
                            {
                                'data': null, 'bSortable': false, 'mRender': function (data, type, full) {
                                    return '<a class="mx-1 btn btn-primary text-secondary" href="SMSRatemanagement.aspx?A=' + full.UserName + '"><i class="fas fa-tasks"></i></a>';
                                }
                            },
    ...
    <script>
        function CheckAccess(username) {
            //var username = 'MIM2000002';
            //alert('HI');
            var result = "";
            $.ajax({
                type: "POST",
                dataType: "text",
                url: "WebService.asmx/IsWABAExist",
                data: { username: username },
                success: function (data) {
                    result = data;
                    if (result == 'NO') {
                        document.getElementById('divWABA').style.display = 'none';
                    }
                    if (result == 'YES') {
                         document.getElementById('divWABA').style.display = '';
                    }
