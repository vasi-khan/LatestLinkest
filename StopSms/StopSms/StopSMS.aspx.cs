...
namespace StopSMS
{
    public partial class StopSMS : System.Web.UI.Page
    {
        //string Segment = "84AB03B", ProfileId = "MIM2000014";
        //string CC = "91";
        //string CC = "971";
        ...
        public void SendOtpFunction()
        {
            ...
            string ccmobileno = CC + Mobileno;
            string ExistsCheck = "SELECT MOBILENO FROM BLACKLISTNO with(nolock) WHERE USERID='" + ProfileId + "' AND MOBILENO='" + ccmobileno.Trim() + "'";
            DataTable dtExist = database.GetDataTable(ExistsCheck);
            ...
            {
                SendOTPthroughAPI(CC, Mobileno.Trim(), dt1.Rows[0]["StopSMS_Userid"].ToString(), Pwd, dt1.Rows[0]["StopSMS_Senderid"].ToString(), dt1.Rows[0]["StopSMS_SMSText"].ToString(), dt1.Rows[0]["StopSMS_Peid"].ToString(), dt1.Rows[0]["StopSMS_Templateid"].ToString(), vOTP);
            }
            Session["SentOtp"] = vOTP;
            string sql1 = @"INSERT INTO LoginEntryStopSMS (UserId,MOBILENO,EntryDate,SENT_OTP) Values('" + ProfileId + "','" + ccmobileno.Trim() + "',CURRENT_TIMESTAMP,'" + vOTP + "')";
        ...
        public void VerifiedOTP()
        {
            ...
            string ccmobileno = CC + Mobileno;
            string otp  = Convert.ToString(database.GetScalarValue(@"SELECT SENT_OTP FROM LoginEntryStopSMS with(nolock) WHERE UserId='" + ProfileId + "' AND MOBILENO='" + ccmobileno.Trim() + "'"));
            if (otp == Convert.ToString(txtotp.Text.Trim()))
            {
                string sql1 = @"UPDATE LoginEntryStopSMS SET SENT_OTP_VERIFY=CURRENT_TIMESTAMP WHERE UserId='" + ProfileId + "' AND MOBILENO='" + ccmobileno.Trim() + "'";
                database.ExecuteNonQuery(sql1);
        ...
        protected void btnUnSubscribe_Click(object sender, EventArgs e)
        {
            ...
            string sql = @"INSERT INTO BLACKLISTNO (USERID,MOBILENO) Values('" + ProfileId + "','" + ccmobileno.Trim() + "')";
