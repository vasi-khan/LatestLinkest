...
namespace StopSMS.Helper
{
    public static class database
    {
        ...
        public static DateTime Setdate(string Cdate)
        {
            ...
            DateTime dt = new DateTime(int.Parse(date[2].ToString()), int.Parse(date[0].ToString()), int.Parse(date[1].ToString()));
            return dt;
        }

        public static DateTime Setdate1(string Cdate)
        {
            char[] a = { ',' };
            string[] date = Cdate.Split('-');
            DateTime dt = new DateTime(int.Parse(date[2].ToString()), int.Parse(date[0].ToString()), int.Parse(date[1].ToString()));
        ...
        public static string GetConnectstringRCS()
        {
            return ConfigurationManager.ConnectionStrings["eMIMRCSLive"].ConnectionString;
        }
        ...
        public static DataTable GetDataTableRCS(string sql)
        {
            DataTable dt = new DataTable();
            using (SqlConnection cnn = new SqlConnection(GetConnectstringRCS()))
            {
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = cnn;
                cmd.CommandTimeout = 3600;
        ...
        public static object GetScalarValueRCS(string sql)
        {
            object o;
            using (SqlConnection cnn = new SqlConnection(GetConnectstringRCS()))
            {
                cnn.Open();
                o = GetScalarValueRCS(sql, cnn);
                cnn.Close();
            }
            return o;
        }
        public static object GetScalarValueRCS(string sql, SqlConnection cnn)
        {
            SqlCommand cmd = new SqlCommand(sql, cnn);
            cmd.CommandTimeout = 3600;
            object obj = cmd.ExecuteScalar();
        ...
        public static void ExecuteNonQueryRCS(string Sql)
        {
        ...
        public static void RemoveSenderID(string[] a)
        {
            for (int i=0;i<a.Length;i++)
            {
                using (SqlConnection cnn = new SqlConnection(GetConnectstring()))
                {
                    try
                    {
                        ...
                        cmd.Connection = cnn;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "Sp_RemoveSenderID";
                        cmd.Parameters.AddWithValue("@id", a[i]);
                        cmd.CommandTimeout = 300;                     
                        cmd.ExecuteNonQuery();
                        cnn.Close();
